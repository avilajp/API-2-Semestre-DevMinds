...
public class ControllerTelaAvaliacao {
    ...
    @FXML
    private TextField nomeAlunoExibicao;
    @FXML
    private TableView<ViewObjAtividadeXAluno> avaliacaoTableView;
    @FXML
    private TableColumn<ViewObjAtividadeXAluno, String> col1;
    @FXML
    private TableColumn<ViewObjAtividadeXAluno, String> col2;
    @FXML
    private TableColumn<ViewObjAtividadeXAluno, String> col3;
    @FXML
    ...
    private void sendToDatabase(ActionEvent event) throws SQLException, ClassNotFoundException {
        ...
        int aux = lista.size();
        for (int i = 0; i < aux; i++) {
            avaliacaoDao.updateAvaliacao(avaliacaoTableView.getItems().get(i).getFeedback(),
                    Double.valueOf(avaliacaoTableView.getItems().get(i).getNota()),
                    avaliacaoTableView.getItems().get(i).getId_avaliacao());
        }
        Alert alert = new Alert(Alert.AlertType.INFORMATION);
        alert.setTitle("Sucesso");
        alert.setHeaderText(null);
        alert.setContentText("Os dados foram salvos no banco com sucesso");
        alert.showAndWait();
    }
